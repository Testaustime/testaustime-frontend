...
export const useLeaderboards = (
  ...
  const leaderboardData = useQueries({
    queries: (leaderboards ?? []).map((leaderboard) => ({
      ...
      placeholderData: initialLeaderboards?.find(
        (l) => l.name === leaderboard.name,
      ),
      enabled: shouldFetch,
    })),
  });

  const { mutateAsync: joinLeaderboard } = useMutation(
    async (leaderboardCode: string) => {
  ...
  const { mutateAsync: leaveLeaderboard } = useMutation(
    async (leaderboardName: string) => {
    ...
    },
    {
      onSuccess: (leaderboardName) => {
        queryClient.setQueriesData(
          ["leaderboards"],
          (leaderboards ?? []).filter(
            (leaderboard) => leaderboard.name !== leaderboardName,
          ),
        );
      },
    },
  ...
  const { mutateAsync: createLeaderboard } = useMutation(
    async (leaderboardName: string) => {
  ...
  const { mutateAsync: deleteLeaderboard } = useMutation(
    async (leaderboardName: string) => {
  ...
  const { mutateAsync: setUserAdminStatus } = useMutation(
    async ({
      leaderboardName,
      adminStatus,
      username,
    ...
    },
    {
      onSuccess: ({ leaderboardName, username, adminStatus }) => {
        const leaderboard = leaderboardData.find(
          (leaderboard) => leaderboard.data?.name === leaderboardName,
        )?.data;
  ...
  const { mutateAsync: kickUser } = useMutation(
    ...
    },
    {
      onSuccess: ({ leaderboardName, username }) => {
        ...
        queryClient.setQueriesData(["leaderboardData", leaderboardName], {
          ...leaderboard,
          members: leaderboard?.members.filter(
  ...
  const { mutateAsync: regenerateInviteCode } = useMutation(
    async (leaderboardName: string) => {
      const res = await axios.post<{ invite_code: string }>(
  ...
  return {
    ...
    deleteLeaderboard,
    promoteUser: (leaderboardName: string, username: string) =>
      setUserAdminStatus({
        leaderboardName,
        username,
        adminStatus: true,
      }),
    demoteUser: (leaderboardName: string, username: string) =>
      setUserAdminStatus({
        leaderboardName,
        username,
        adminStatus: false,
    ...
    setUserAdminStatus,
    kickUser: (leaderboardName: string, username: string) =>
      kickUser({ leaderboardName, username }),
    regenerateInviteCode: async (leaderboardName: string) => {
      return (await regenerateInviteCode(leaderboardName)).inviteCode;
    },
  };
};
