...
import { VictoryTheme, VictoryAxis, VictoryChart, VictoryGroup, VictoryLine, VictoryScatter, VictoryTooltip, VictoryVoronoiContainer, VictoryThemeDefinition, VictoryLabel } from "victory";
...
export const DailyCodingTimeChart = ({ entries, dayCount }: DailyCodingTimeChartProps) => {
  const usesDarkMode = useMantineTheme().colorScheme === "dark";

  const data = Array(dayCount).fill(0).map((_, dayIndex) => {
    const date = startOfDay(addDays(new Date(), -dayIndex));
    const duration = sumBy(entries.filter(entry => entry.dayStart.getTime() === date.getTime()), entry => entry.duration);
  ...
  return <VictoryChart
    theme={!usesDarkMode ? { ...VictoryTheme.grayscale, axis: { style: { tickLabels: { fontSize: 11 } } } } : darkVictoryTheme}
    ...
    <VictoryGroup
      ...
      labels={(d) => `${format(d.datum.x, "d.M.yyyy")}\n${prettyDuration(d.datum.y)}`}
      labelComponent={
        <VictoryTooltip
